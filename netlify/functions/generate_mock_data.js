...
Верни ТОЛЬКО JSON:
{
  "inserts": "ОДНА строка текста (НЕ объект, НЕ массив!) со всеми INSERT INTO командами для ВСЕХ таблиц из TABLE_COUNTS. Только SQL, без markdown, без пояснений, без оценок и рецензий.",
  "notes": "Максимум 2 коротких предложения: порядок FK и допущения. Запрещены эссе, списки «сильных сторон», оценки 10/10, советы про multi-row как отдельная статья."
}
...
- Предпочитай multi-row INSERT: один INSERT INTO t (cols) VALUES (...), (...), (...); на таблицу, а не десятки одиночных INSERT.
...
- Поле inserts — только SQL INSERT. Поле notes — только краткая справка (не «разбор скрипта»).
...
function looksLikeEssayNotSql(text) {
  ...
  if (/оценка:\s*\d+\s*\/\s*10|10\s*\/\s*10|безупречно|excellent|золотой стандарт|сильные стороны|итоговый вердикт|профессиональный совет/i.test(s)) return true;
